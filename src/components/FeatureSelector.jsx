...
const FeatureSelector = ({ vars, X, setX, y, sety }) => {   //* FeatureSelector component | ACCEPTS: vars, selectedFeatures, and setSelectedFeatures props
  ...
  return (
    <div className='mt-10'>
      <label className='font-bold'>Columns ({vars.length})</label>
      <button 
        onClick={() => setX([])} 
      ...
      <div className='flex flex-wrap'>
        {vars.map((col, index) => (
          <div 
            key={index} 
            className={`border p-2 m-1 cursor-pointer ${X.includes(col) ? 'bg-gray-300' : ''} ${y.includes(col) ? 'bg-red-300' : ''}`} 
      ...
      <h3><span className='font-bold'>Select Features</span> <span className='text-gray-400'>(independent variables)</span></h3>
      <Select   
        className='w-fit'
      ...
      />
      <h3><span className='font-bold'>Select Label </span><span className='text-gray-400'>(dependent variable)</span></h3>
      <Select  
        className='w-fit' 
        options={options}               // Pass the options
        isMulti                         // Allow multiple selections
